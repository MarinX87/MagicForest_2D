...
HRESULT InitEnemy(void)
{
	...
	for (int i = 0; i < TEXTURE_MAX; i++)
	{
		for (int j = 0; j < ENEMY_MAX; j++)
		{
			...
			g_Enemy[i][j].lastHitTime = -1000.0f;
			g_Enemy[i][j].invincibilityDuration = 800; 
			g_Enemy[i][j].isBack = FALSE;
			g_Enemy[i][j].freezeStartTime = 0;
			g_Enemy[i][j].isFrozen = FALSE;
			g_Enemy[i][j].liveTime = 0;
			g_Enemy[i][j].hitTime = 0;

			//ZOMBIEの特別制定
			if (i == ZOOBIE)                                        
			{
				g_Enemy[i][j].blood = 3;                            //ブラッド
				g_Enemy[i][j].move = XMFLOAT3(0.3f, 0.0f, 0.0f);    //移動速度
			...
			if (i == PUMPKIN)                                       
			{
				g_Enemy[i][j].blood = 6;                            //ブラッド
				g_Enemy[i][j].move = XMFLOAT3(0.8f, 0.0f, 0.0f);	//移動速度
				g_Enemy[i][j].attack = 1;							//攻撃力
...
void UpdateEnemy(void)
{
	...
	if ((relNow % 5 == 0)  &&  (relNow  != 0))
	{
		...
		{
			(int)(ENEMY_MAX * 0.2f),
			relNow > GAME_TIME*0.25f ? (int)(ENEMY_MAX * 0.3f) : 0,
			relNow > GAME_TIME*0.5f  ? (int)(ENEMY_MAX * 0.6f) : 0,
			relNow > GAME_TIME*0.75f ? ENEMY_MAX : 0,
			relNow > GAME_TIME*1.2f  ? ENEMY_MAX*2 : 0
		};
	...
	for (int i = 0; i < TEXTURE_MAX; i++)
	{
		for (int j = 0; j < ENEMY_MAX; j++)
		{
			if (g_Enemy[i][j].use == TRUE)
			{
				...
				if (relNow - g_Enemy[i][j].liveTime >= 45)
				{
					g_Enemy[i][j].use = FALSE;
				}
				...
				//凍ってる判定
				if (g_Enemy[i][j].isFrozen)
				{
					g_Enemy[i][j].blood = 0.1f;

					if (relNow - g_Enemy[i][j].freezeStartTime > 5)  
					{
						g_Enemy[i][j].isFrozen = FALSE; 
						g_Enemy[i][j].freezeStartTime = 0.0f;
						g_Enemy[i][j].blood = 3;
					}
					continue;
				}
				...
				// 当たり
				if (g_Enemy[i][j].isHit)
				{

					if (g_Enemy[i][j].isBack)
					{
						// 後退値
						float retreatDistance = 40.0f;

						switch (g_Enemy[i][j].dir)
						{
						case CHAR_DIR_UP:
							g_Enemy[i][j].pos.y += retreatDistance;
							break;
						case CHAR_DIR_DOWN:
							g_Enemy[i][j].pos.y -= retreatDistance;
							break;
						case CHAR_DIR_LEFT:
							g_Enemy[i][j].pos.x += retreatDistance;
							break;
						case CHAR_DIR_RIGHT:
							g_Enemy[i][j].pos.x -= retreatDistance;
							break;
					...
					// 当たったらフラシュ
					if (GetRelativeTime() - g_Enemy[i][j].hitTime > 500)
					{
						g_Enemy[i][j].isHit = FALSE;
					}
	...
	for (int i = 0; i < TEXTURE_MAX; i++)
	{
		for (int j = 0; j < ENEMY_MAX; j++)
		{
			if (g_Enemy[i][j].use == TRUE)		
			{
				...
				SetSpriteColorRotation(g_VertexBuffer, px, py, pw, ph, tx, ty, tw, th, color, g_Enemy[i][j].rot.z);
...
void SpawnEnemiesAtRandomPosition(PLAYER* player, int enemyType, int countToSpawn)
{
	...
	for (int i = 0; i < ENEMY_MAX && spawned < countToSpawn; i++)
	{
		if (!g_Enemy[enemyType][i].use)
		{
			...
			g_Enemy[enemyType][i].pos.x = player->pos.x + radiusX * cosf(angle) + 250.0f;
			g_Enemy[enemyType][i].pos.y = player->pos.y + radiusY * sinf(angle) + 250.0f;
			g_Enemy[enemyType][i].isHit = FALSE;
			g_Enemy[enemyType][i].liveTime = GetTime();

			switch (enemyType)
			{
			case ZOOBIE:
				g_Enemy[enemyType][i].blood = 2.5f;
				break;
			case GHOST:
				g_Enemy[enemyType][i].blood = 4;
				break;
			case PUMPKIN:
				g_Enemy[enemyType][i].blood = 6;
				break;
			case SPRITE:
				g_Enemy[enemyType][i].blood = 4;
				break;
			...
			if (relNow >= GAME_TIME)
			{
				...
				{
				case ZOOBIE:
					g_Enemy[enemyType][i].blood = 8;
					g_Enemy[enemyType][i].move = XMFLOAT3(1.0f, 0.0f, 0.0f);
					break;
				case GHOST:
					g_Enemy[enemyType][i].blood = 8;
					g_Enemy[enemyType][i].move = XMFLOAT3(1.5f, 0.0f, 0.0f);
					break;
				case PUMPKIN:
					g_Enemy[enemyType][i].blood = 8;
					g_Enemy[enemyType][i].move = XMFLOAT3(1.5f, 0.0f, 0.0f);
				...
				case SPRITE:
					g_Enemy[enemyType][i].blood = 8;
					g_Enemy[enemyType][i].move = XMFLOAT3(2.0f, 0.0f, 0.0f);
					break;
				default:
					g_Enemy[enemyType][i].blood = 8;
					g_Enemy[enemyType][i].move = XMFLOAT3(2.0f, 0.0f, 0.0f);
