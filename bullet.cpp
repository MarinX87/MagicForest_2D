...
#define BOOK_RADIUS                     (130)   // 回す半径
#define BOOK_ROTATION_SPEED             (0.03f) // 回すスピード


//FIRE
#define TEXTURE_WIDTH_FIRE              (180)   // キャラサイズ
#define TEXTURE_HEIGHT_FIRE				(120)   // キャラサイズ
#define FIRE_MAX                         (5)    // 最大値
#define FIREBALL_SPEED                   (8)    // スピード
...
//タイマー(攻撃頻度用)
static int g_LastAxeThrowTime;
static int g_LastFireThrowTime;
static int g_LastLightningThrowTime;
...
{
	NowTime = 0;
	g_LastAxeThrowTime = 0;
	g_LastFireThrowTime = 0;
	g_LastLightningThrowTime = 0;
	...
	for (int i = 0; i < AXE_MAX; i++)
	{
		...
		g_Axe[i].pos   = XMFLOAT3(30000.0f, 0.0f, 1.0f);
	...
	for (int i = 0; i < BOOK_MAX; i++)
	{
		...
		g_Book[i].pos   = XMFLOAT3(30000.0f, 0.0f, 1.0f);   
	...
	for (int i = 0; i < FIRE_MAX; i++)
	{
		...
		g_Fire[i].pos   = XMFLOAT3(30000.0f, 0.0f, 1.0f);
	...
	for (int i = 0; i < LIGHTING_MAX; i++)
	{
		...
		g_Lighting[i].pos         = XMFLOAT3(30000.0f, 0.0f, 1.0f);
...
void UpdateBullet(void)
{
	...
	//攻撃の頻度は5ｓ／１回
	if (currentTime_AXE - g_LastAxeThrowTime >= 5)
	{
			...
			// エネミーの当たり判定処理
			{
				...
				for (int j = 0; j < 4; j++)
				{
					for (int k = 0; k < ENEMY_MAX; k++)
					{
						...
						{
							// エネミー無敵状態？
							if (GetRelativeTime() - enemy[j][k].lastHitTime < enemy[j][k].invincibilityDuration)
							{
								continue; 
							}
							...
							if (ans == TRUE)
							{
								enemy[j][k].isHit = TRUE;
								enemy[j][k].hitTime = GetRelativeTime();
								enemy[j][k].lastHitTime = GetRelativeTime();

								enemy[j][k].blood -= g_Axe[i].atk;
								if (enemy[j][k].blood <= 0)
								{
									enemy[j][k].use = FALSE;
					...
					for (int j = 0; j < 4; j++) {
						for (int k = 0; k < ENEMY_MAX; k++) 
						{
							if (enemy[j][k].use == TRUE) 
							{
								...
								{
									enemy[j][k].isHit = TRUE;
									enemy[j][k].isBack = TRUE;
									enemy[j][k].hitTime = GetRelativeTime();
									enemy[j][k].lastHitTime = GetRelativeTime();

									enemy[j][k].blood -= g_Book[i].atk;
									if (enemy[j][k].blood <= 0) 
									{
										enemy[j][k].use = FALSE;
										// 撃破カウンター
	...
	//攻撃の頻度は4ｓ／１回
	if (currentTime_FIRE - g_LastFireThrowTime >= 4)
	{
		float maxAngle = 0.0f;
		int maxEnemies = 0;
		float sectorSize = XMConvertToRadians(30.0f);

		for (int degree = 0; degree < 360; degree++)
		{
			float angle = XMConvertToRadians((float)degree);
			int enemyCount = CountEnemiesInDirection(angle, player, sectorSize);
		...
		const float angleOffset = XM_PI / 180.0f * 5.0f;  
		...
		for (int i = 0; i < FIRE_MAX; i++)
		{
			if (g_Fire[i].use == TRUE)
			{
				...
				g_Fire[i].angle = maxAngle + currentOffset;
...
int CountEnemiesInDirection(float angle, PLAYER* player, float sectorSize)
{
	...
	for (int j = 0; j < 4; j++)
	{
		for (int k = 0; k < ENEMY_MAX; k++)
		{
			if (enemy[j][k].use == TRUE)
			{
				float dx = enemy[j][k].pos.x - player->pos.x;
				float dy = enemy[j][k].pos.y - player->pos.y;

				
				float enemyAngle = atan2f(dy, dx);
				if (enemyAngle < 0)
					enemyAngle += 2.0f * M_PI;

				float angleDifference = fabs(enemyAngle - angle);

				
				if (angleDifference > M_PI)
					angleDifference = 2.0f * M_PI - angleDifference;

				if (angleDifference < sectorSize / 2.0f)
				{
					count++;
				}
			}
