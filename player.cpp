...
HRESULT InitPlayer(void)
{
	...
	{
		g_Player[i].use = TRUE;
		g_Player[i].pos = XMFLOAT3(1500.0f,1500.0f , 0.0f);	// 中心点から表示
...
void UpdatePlayer(void)
{
	for (int i = 0; i < PLAYER_MAX; i++)
	{
		if (g_Player[i].use == TRUE)
		{
			...
			{
				BOOL ansL = CollisionBB(g_Player[i].pos, g_Player[i].w, g_Player[i].h,
					rangeL[a].pos, rangeL[a].w, rangeL[a].h);
				if (ansL == TRUE)
				{
			...
			{
				BOOL ansM = CollisionBB(g_Player[i].pos, g_Player[i].w, g_Player[i].h,
					rangeM[b].pos, rangeM[b].w, rangeM[b].h);
				if (ansM == TRUE)
				{
					g_Player[i].pos = pos_old;
				}
			...
			for (int c = 0; c < BOX_MAX; c++)
			{
				BOOL ansR = CollisionBB(g_Player[i].pos, g_Player[i].w, g_Player[i].h,
					rangeR[c].pos, rangeR[c].w, rangeR[c].h);
				if (ansR == TRUE)
				{
					g_Player[i].pos = pos_old;
				}
			...
			//宝箱をもらう
			BG* box = GetBox();
			for (int d = 0; d < BOX_MAX; d++)
			{
				if (box[d].use == TRUE)
				{
					BOOL ansR = CollisionBB(g_Player[i].pos, g_Player[i].w, g_Player[i].h,
						box[d].pos, box[d].w, box[d].h);
					if (ansR == TRUE && g_Player[i].boxTouch == FALSE)
					{
						box[d].use = FALSE;
						box[d].start = TRUE;
						g_Player[i].boxTouch = TRUE;
						g_Player[i].blood = 7;
						break;
			...
			for (int j = 0; j < 4; j++)
			{
				for (int k = 0; k < ENEMY_MAX; k++)
				{
					...
					if (enemy[j][k].use == TRUE)
					{
						...
						{
							//当たり判定関数で
							BOOL ans = CollisionBB(g_Player[i].pos, g_Player[i].w - 25.0f, g_Player[i].h - 25.0f,
								enemy[j][k].pos, enemy[j][k].w, enemy[j][k].h);

							// 当たっている？
							if (ans == TRUE && enemy[j][k].isFrozen == FALSE)
							{
								...
								g_Player[i].flashStartTime = GetRelativeTime();
				...
				//　無敵状態の時間を計算する
				if (g_Player[i].flashing && (GetRelativeTime() - g_Player[i].flashStartTime >= 800))
				{
...
void DrawPlayer(void)
{
	...
	for (int i = 0; i < PLAYER_MAX; i++)
	{
		...
		if (g_Player[i].use == TRUE)
		{
            ...
            if (g_Player[i].flashing)
            {
                DWORD currentTime = GetRelativeTime();
